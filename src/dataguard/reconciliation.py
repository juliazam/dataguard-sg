...
def reconcile_counts(rows_fetched: int, rows_valid: int, rows_invalid: int) -> dict:
    ...
    if is_reconciled:
        logger.info('Data reconciliation successful. All %d records accounted for '
                    '(%d valid, %d invalid).',
                    rows_fetched, rows_valid, rows_invalid)
    else:
        logger.error('Data reconciliation failed! Critical error in pipeline. '
        'Fetched: %d, Accounted (Valid + Invalid): %d. Discrepancy: %d rows.',
        rows_fetched, rows_accounted, discrepancy)
    return {
