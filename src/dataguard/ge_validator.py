...
def run_expectations(df: pd.DataFrame) -> dict:
    ...
    suite.add_expectation(
        ge.expectations.ExpectColumnValuesToNotBeNull(column="Record_ID")
    )
    ...
    suite.add_expectation(
        ge.expectations.ExpectColumnValuesToNotBeNull(column="Date_of_Payment")
    )
    ...
    validation_definition = context.validation_definitions.add(
        ge.ValidationDefinition(
            name="payments_validation", data=batch_definition, suite=suite
        )
    )

    checkpoint = context.checkpoints.add(
        ge.Checkpoint(
            name="payments_checkpoint", validation_definitions=[validation_definition]
        )
    )
