...
def test_run_pipeline_creates_output_files(monkeypatch, tmp_path, fake_api_df):
    ...
    assert (tmp_path / "lineage_log.jsonl").exists(), "Файл lineage_log.jsonl не создался"

    csv_files = list(tmp_path.glob("validated_payments_*.csv"))
    assert len(csv_files) >= 1, "CSV файл по паттерну validated_payments_*.csv не найден"
